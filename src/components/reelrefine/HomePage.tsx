...
export type SortOption = "latest" | "alphabetical" | "random";
...
export const HomePage: React.FC<HomePageProps> = ({
  ...
  const filteredProjects = React.useMemo(() => {
    let list = SAMPLE_PROJECTS.filter((proj) => {
  ...
  return (
    <div ref={containerRef} className="space-y-8 font-sans max-w-7xl mx-auto pb-8 relative">
      ...
      {/* Search Bar with Filter Button inside right edge */}
      <div className="relative flex items-center" ref={filterRef}>
        <Search className="h-4 w-4 absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none z-10" />
        <input
          type="text"
          value={searchQuery}
          onChange={(e) => setSearchQuery(e.target.value)}
          placeholder="Search movies by title, author, genre, or keyword..."
          className="w-full pl-11 pr-44 py-3 text-sm bg-card border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-[#001b94] dark:focus:ring-sky-400 focus:border-[#001b94] dark:focus:border-sky-400 placeholder:text-slate-400 font-medium transition-all shadow-xs text-foreground"
        />
        {searchQuery && (
          <button
            type="button"
            onClick={() => setSearchQuery("")}
            className="absolute right-36 top-1/2 -translate-y-1/2 p-1 rounded-md text-slate-400 hover:text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors z-10"
            title="Clear search"
          >
            <X className="w-4 h-4" />
          </button>
        ...
        {/* Filter Dropdown Button on Rightmost side inside Searchbar */}
        <div className="absolute right-2 top-1/2 -translate-y-1/2 z-20">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-all cursor-pointer border ${
              isFilterOpen || sortBy !== "latest"
                ? "bg-[#001b94] text-white border-[#001b94] shadow-xs"
                : "bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border-border hover:bg-slate-200 dark:hover:bg-slate-700"
            }`}
            title="Sort and filter screenplays"
          >
            <SlidersHorizontal className="w-3.5 h-3.5" />
            <span className="capitalize">
              {sortBy === "latest" ? "Latest" : sortBy === "alphabetical" ? "A-Z" : "Random"}
            </span>
          ...
          {isFilterOpen && (
            <div className="absolute right-0 mt-2 w-48 bg-card border border-border rounded-xl shadow-xl py-1.5 z-50 text-xs font-medium">
              <div className="px-3 py-1.5 text-[10px] font-mono font-semibold uppercase tracking-wider text-muted-foreground border-b border-border mb-1">
                Order Movies By
              </div>
              ...
              <button
                ...
                className={`w-full text-left px-3 py-2 flex items-center justify-between hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors ${
                  sortBy === "latest" ? "text-[#001b94] dark:text-sky-400 font-semibold bg-[#001b94]/5 dark:bg-sky-400/10" : "text-foreground"
                }`}
              >
                <span>Latest First</span>
                {sortBy === "latest" && <Check className="w-3.5 h-3.5 text-[#001b94] dark:text-sky-400" />}
              </button>
              ...
              <button
                ...
                className={`w-full text-left px-3 py-2 flex items-center justify-between hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors ${
                  sortBy === "alphabetical" ? "text-[#001b94] dark:text-sky-400 font-semibold bg-[#001b94]/5 dark:bg-sky-400/10" : "text-foreground"
                }`}
              >
                <span>Alphabetical (A-Z)</span>
                {sortBy === "alphabetical" && <Check className="w-3.5 h-3.5 text-[#001b94] dark:text-sky-400" />}
              </button>
              ...
              <button
                ...
                className={`w-full text-left px-3 py-2 flex items-center justify-between hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors ${
                  sortBy === "random" ? "text-[#001b94] dark:text-sky-400 font-semibold bg-[#001b94]/5 dark:bg-sky-400/10" : "text-foreground"
                }`}
              >
                <span>Random Order</span>
                {sortBy === "random" && <Check className="w-3.5 h-3.5 text-[#001b94] dark:text-sky-400" />}
              </button>
            </div>
          )}
        </div>
      ...
      {showScrollTop && (
        <button
          type="button"
          onClick={scrollToTop}
          className="fixed bottom-5 left-1/2 -translate-x-1/2 z-50 px-3.5 py-1.5 bg-white/40 dark:bg-slate-900/70 backdrop-blur-md border border-white/40 dark:border-white/20 text-slate-900 dark:text-white font-semibold text-[11px] uppercase tracking-wide rounded-lg shadow-lg hover:bg-white/60 dark:hover:bg-slate-800 hover:border-amber-400/60 hover:scale-105 active:scale-95 transition-all duration-300 flex items-center gap-1.5 cursor-pointer group"
          title="Scroll back to top"
          aria-label="Scroll back to top"
        >
          <ChevronUp className="w-3.5 h-3.5 text-amber-500 dark:text-amber-400 group-hover:-translate-y-0.5 transition-transform" />
